...
class OpenFOAMFieldParser:
    ...
    def parse_scalar_field(self, field_path):
        ...
        try:
            ...
            internal_match = re.search(r'internalField\s+(?:uniform\s+)?([^;]+);', content, re.DOTALL)
    ...
    def parse_vector_field(self, field_path):
        ...
        try:
            ...
            if 'uniform' in content:
                ...
                vec_match = re.search(r'\(\s*([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s*\)', field_data)
                if vec_match:
                    return float(vec_match.group(1)), float(vec_match.group(2)), float(vec_match.group(3))
                return None, None, None

            # Handle nonuniform field
            if 'nonuniform' in content:
                # Extract all vectors
                vectors = re.findall(r'\(\s*([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s+([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)\s*\)', field_data)
                if vectors:
    ...
    def get_latest_time_data(self):
        ...
        scalar_fields = ['p', 'nut', 'nuTilda', 'k', 'epsilon', 'omega', 'T', 'alpha.water']
    ...
    def calculate_pressure_coefficient(self, p_field, p_inf=101325, rho=1.225, u_inf=1.0):
    ...
    def get_residuals_from_log(self, log_file='log.foamRun'):
        ...
        try:
            with open(log_path, 'r') as f:
                for line in f:
                    # Parse time
                    time_match = re.search(r'Time\s*=\s*([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)', line)
                    if time_match:
                    ...
                    for field in ['Ux', 'Uy', 'Uz', 'p', 'k', 'epsilon', 'omega']:
                        residual_match = re.search(rf'{field}.*Initial residual\s*=\s*([-+]?\d*\.?\d+(?:[eE][-+]?\d+)?)', line)
