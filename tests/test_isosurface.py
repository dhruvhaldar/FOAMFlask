...
class TestGetInteractiveHtmlStaticIsosurfaces:
    ...
    def test_get_interactive_html_temp_file_cleanup_failure(self, visualizer, temp_vtk_file, mocker, caplog):
        ...
        assert mock_plotter.close.call_count >= 1
...
class TestCleanup:
    ...
    def test_del_handles_exception(self, mocker, caplog):
        ...
        # Verify the initialization message was logged and the cleanup error was logged as a warning
        # 1. Initialized INFO
        # 2. Error during cleanup WARNING
        assert len(caplog.records) == 2
        assert caplog.records[1].levelname == "WARNING"
        assert "Error during cleanup" in caplog.records[1].message
