    ...
    try {
        ...
        if (traces.length > 0) {
            ...
            if (residualsPlotDiv) {
                ...
                void Plotly.react(residualsPlotDiv, [traces], layout, { ...plotConfig, displayModeBar: true, scrollZoom: false }).then(() => attachWhiteBGDownloadButton(residualsPlotDiv));
            }
        }
    }
    catch (err) {
...
const updatePlots = async () => {
    ...
    try {
        ...
        if (data.p && data.time) {
            ...
            const plotData = window.Plotly.d3.select(`#${pressureDiv.id}`).data()[0];
