...
## 2026-03-01 - [Regex Lookahead vs Lazy Match Performance]
**Learning:** In Python's `re` module, replacing greedy negative lookaheads like `(?:(?!\1).)+` with lazy quantifiers like `.*?` prevents O(N^2) catastrophic backtracking and significantly improves matching speed on long strings (e.g., path sanitization in `backend/utils.py`).
**Action:** When a regular expression is designed to capture everything up to the very next occurrence of a delimiter (like a closing quote), prefer a lazy match `.*?` over complex negative lookaheads to improve both readability and performance.
