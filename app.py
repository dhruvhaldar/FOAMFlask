...
def is_safe_case_root(path_str: str) -> bool:
    ...
    Also blocks hidden directories and the user's home directory.
    ...
    try:
        ...
        # Security: Prevent setting root to hidden directories (e.g. ~/.ssh)
        # Iterate over parts to find any hidden component
        for part in path_obj.parts:
            if part.startswith(".") and part != ".":
                return False

        # Security: Prevent setting root to user's home directory directly
        # This prevents cluttering the home dir and exposing all personal files
        try:
            home = Path.home().resolve()
            # Check if path is exactly home
            if path_obj.resolve() == home:
                return False
        except (RuntimeError, OSError):
            # If home cannot be determined, proceed with other checks
            pass
...
def set_security_headers(response: Response) -> Response:
    ...
    if not request.cookies.get("csrf_token"):
        ...
        response.set_cookie(
            "csrf_token", secrets.token_hex(32), samesite="Lax", secure=False
        )
...
def index() -> str:
    ...
    options_html = "\n".join(
        f'<option value="{escape(t)}">{escape(t)}</option>' for t in tutorials
    )
...
def api_view_geometry() -> Union[Response, Tuple[Response, int]]:
    ...
    html_content = GeometryVisualizer.get_interactive_html(
